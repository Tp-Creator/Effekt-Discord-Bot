...
def main():
    ...
    if os.path.isfile("logs/latest.log"):
        new_file_name = datetime.datetime.fromtimestamp(
            os.path.getctime("logs/latest.log")
        ).strftime("%Y-%m-%d-")
        index = 1
        if os.path.exists(f"logs/{new_file_name}{index}.log"):
            while os.path.exists(f"logs/{new_file_name}{index}.log"):
                index += 1
        os.rename("logs/latest.log", f"logs/{new_file_name}{index}.log")
    ...
    if os.path.exists("config.json"):
        ...
        try:
            ...
            with open("config.json", "w") as file:
                json.dump(
                    configuration, file, sort_keys=True, indent=4, ensure_ascii=False
                )

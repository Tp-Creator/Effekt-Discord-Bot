            ...
            if packetID == 0:
                ...
                if state == 1:
                    self.logger.info(
                        (
                            "[%s:%s] Received client "
                            + ("(using ForgeModLoader) " if is_using_fml else "")
                            + "ping packet (%s:%s)."
                        )
                    ...
                    self.logger.info(
                        (
                            "[%s:%s] "
                            + (name + " t" if len(name) > 0 else "T")
                            + "ries to connect to the server "
                            + ("(using ForgeModLoader) " if is_using_fml else "")
                            + "(%s:%s)."
                        )
                        % (client_ip, addr[1], ip, port)
                    ...
                    self.write_response(
                        client_socket, json.dumps({"text": self.kick_message})
                    )
                else:
                    self.logger.info(
                        "[%s:%d] Tried to request a login/ping with an unknown state: %d"
                ...
                self.logger.warning(
                    "[%s:%d] Sent an unexpected packet: %d"
